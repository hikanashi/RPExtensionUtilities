...
public class RPEventBridge extends ARPBridge {
    ...
    public boolean isUpdate(IRPModelElement element) {
        ...
        if(doxygen_.getName().equals(rpevent.getName()) != true) {
            debug("Event Name is change "+ rpevent.getName() + "->" + doxygen_.getName());
            return true;
        }
    ...
    public boolean isUpdateArgment(String eventName, IRPArgument rpArgment, DoxygenType type) {
        DoxygenTypeParam param = getObject(type);

        if(param.getName().equals(rpArgment.getName()) != true) {
            debug(eventName + " Argment Name is change "+ rpArgment.getName() + "->" + param.getName());
            return true;
        }

        if(param.getDirection().equals(rpArgment.getArgumentDirection()) != true) {
            debug(eventName + " Direction is change "+ rpArgment.getArgumentDirection() + "->" + param.getDirection());
            return true;
        }
    ...
    public void applyByType(IRPModelElement element, String currentVersion) {
        ...
        if(doxygen_.getName().equals(rpevent.getName()) != true) {
            debug("Event Name is apply "+ rpevent.getName() + "->" + doxygen_.getName());
        ...
        for(DoxygenType value : params ) {
            ...
            if( find_index >= 0) {
                ...
                debug(String.format("Event:%s argment:%s delete(from:%d, to:%d)",
                        rpevent.getDisplayName() , 
                        rpArgment.getName(),
                        argument_index, delete_count));
            } 
            else if( argument_index < args.size()  ) {
                debug(String.format("Event:%s add(%s, %d)",
                ...
                debug(String.format("Event:%s add(%s, last)",
        ...
        debug(String.format("Event:%s delete(from:%d, to:%d)",
                rpevent.getDisplayName() , 
                argument_index, delete_count));
